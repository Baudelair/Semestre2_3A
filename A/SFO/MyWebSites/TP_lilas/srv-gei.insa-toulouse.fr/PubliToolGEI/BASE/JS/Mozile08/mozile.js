...
mozile.findRoot=function(){var scripts=document.getElementsByTagName("script");for(var i=0;i<scripts.length;i++){if(scripts[i].getAttribute("src")!=null && scripts[i].getAttribute("src").indexOf("mozile.js")>-1){var root=scripts[i].getAttribute("src");root=root.substring(0,root.lastIndexOf("mozile.js"));if(root.indexOf("src")>-1)
root=root.substring(0,root.lastIndexOf("src"));return root;}}
return"";}
...
mozile.require=function(moduleName){if(!mozile.findModule(moduleName)){if(mozile.precompiled)return false;return mozile.loadModule(moduleName);}
else return false;}
...
mozile.getDirectory=function(path){if(!path)return"";path=path.toString();path=mozile.getPath(path);if(path.indexOf(mozile.filesep)>-1){if(path.indexOf(":///")>-1){var partial=path.substring(path.indexOf(":///")+4);if(partial.indexOf(mozile.filesep)==-1)return path;}
else if(path.indexOf("://")>-1){var partial=path.substring(path.indexOf("://")+3);if(partial.indexOf(mozile.filesep)==-1)return path;}
return path.substring(0,path.lastIndexOf(mozile.filesep)+1);}
return path;}
mozile.getAbsolutePath=function(path,root){if(!path)return"";path=path.toString();if(mozile.isPathAbsolute(path))return path;if(!root)root=location;root=mozile.getDirectory(root);root=mozile.getAbsolutePath(root);return root+path;}
...
mozile.dumpError=function(object){if(typeof(object)=="string")return object;if(!mozile.browser.isIE)return object.toString();if(object&&object.description){var fields=["Name: "+object.name,"Number: "+object.number,"Message: "+object.message,"Description: "+object.description,];return fields.join("\n");}
if(!object)object=document;if(!object.parseError)object=document;if(!object.parseError)object=window;if(!object.parseError)return"[No error to parse]";var fields=["Error Code: "+object.parseError.errorCode,"File Position: "+object.parseError.filepos,"Line: "+object.parseError.line,"Line Position: "+object.parseError.linepos,"Reason: "+object.parseError.reason,"Source Text: "+object.parseError.srcText,"Url: "+object.parseError.url];return fields.join("\n");}
mozile.editElement=function(elementOrId){var element;if(typeof(elementOrId)=="string"){if(document.documentElement.nodeName.toLowerCase()=="html")
element=document.getElementById(elementOrId);else return mozile.editElements(elementOrId,"id",true);}
else if(elementOrId.nodeType&&elementOrId.nodeType==mozile.dom.ELEMENT_NODE){element=elementOrId;}
if(element){mozile.require("mozile.edit");mozile.edit.setMark(element,"editable",true);switch(element.getAttribute("contentEditable")){case"true":mozile.edit.setMark(element,"contentEditable",true);break;case"false":mozile.edit.setMark(element,"contentEditable",false);break;}
if(mozile.browser.isIE){element.setAttribute("contentEditable","true");}}
return undefined;}
mozile.editElements=function(listOrValue,name,single){mozile.require("mozile.dom");var list=new Array();if(name===undefined)name="class";if(typeof(listOrValue)=="string"){if(document.createTreeWalker){var treeWalker=document.createTreeWalker(document.documentElement,mozile.dom.NodeFilter.SHOW_ELEMENT,null,false);while(treeWalker.nextNode()){var node=treeWalker.currentNode;if(name=="class"&&mozile.browser.isIE&&node.className==listOrValue)list.push(node);else if(name&&node.getAttribute(name)==listOrValue)list.push(node);else if(!name&&mozile.dom&&mozile.dom.getLocalName(node).toLowerCase()==listOrValue){list.push(node);}
if(single&&list.length>0)break;}}}
else if(listOrValue.length){list=listOrValue;}
if(list.length){for(var i=0;i<list.length;i++){if(list[i]&&list[i].nodeType&&list[i].nodeType==mozile.dom.ELEMENT_NODE){mozile.editElement(list[i]);if(single)break;}}}
return undefined;}
mozile.editDocument=function(doc){if(!doc)doc=document;mozile.editElement(doc.documentElement);}
mozile.editAllText=function(rich){mozile.require("mozile.edit");mozile.edit.addDefaultCommand(mozile.edit.navigateLeftRight);mozile.edit.addDefaultCommand(mozile.edit.insertText);mozile.edit.addDefaultCommand(mozile.edit.removeText);if(rich){mozile.require("mozile.edit.rich");mozile.edit.addDefaultCommand(mozile.edit.remove);var splitBlocks=new mozile.edit.Split("splitBlocks");splitBlocks.accel="Return Enter";mozile.edit.addDefaultCommand(splitBlocks);}}
...
return null;}
mozile.dom._matchNonWhitespace=/\S/;mozile.dom.isWhitespace=function(node){if(!node||!node.nodeValue)return false;if(node.nodeValue.match(mozile.dom._matchNonWhitespace))return false;return true;}
...
mozile.dom.getNamespaceURI=function(node){if(node.namespaceURI)return node.namespaceURI;else if(node.nodeName.indexOf(":")>-1){var prefix=node.nodeName.substring(0,node.nodeName.indexOf(":"));return mozile.dom.lookupNamespaceURI(node,prefix);}
...
else link=mozile.dom.createElement("link");link.setAttribute("class","mozileLink");link.setAttribute("href",href);link.setAttribute("rel","stylesheet");if(!type)type="text/css";link.setAttribute("type",type);mozile.dom.getHead().appendChild(link);mozile.dom.links.push(link);}
mozile.dom.getX=function(obj,container){var x=0;if(obj.offsetParent){while(obj.offsetParent){x+=obj.offsetLeft;obj=obj.offsetParent;if(obj==container)break;}}
else if(obj.x)x+=obj.x;return x;}
mozile.dom.getY=function(obj,container){var y=0;if(obj.offsetParent){while(obj.offsetParent){y+=obj.offsetTop;obj=obj.offsetParent;if(obj==container)break;}}
else if(obj.y)y+=obj.y;return y;}
mozile.dom.selection=new Object();mozile.dom.selection.prototype=new mozile.Module;mozile.dom.selection.get=function(){if(!mozile.dom._selection)mozile.dom._selection=new mozile.dom.Selection();else if(mozile.browser.isIE)mozile.dom._selection._init();return mozile.dom._selection;}
...
return null;}
mozile.dom.Range=function(selectRange){if(document.createRange){var range;if(selectRange)range=selectRange.cloneRange();else range=document.createRange();for(var field in mozile.dom.Range.prototype)
range[field]=mozile.dom.Range.prototype[field];return range;}else{if(selectRange&&selectRange._range){var range=new mozile.dom.InternetExplorerRange(selectRange._range.duplicate());range._init();return range;}
...
catch(e){try{var xmlDoc=new ActiveXObject("Microsoft.XMLDOM");xmlDoc.async=false;try{var loaded=xmlDoc.load(uri);if(loaded)return xmlDoc;else{mozile.debug.inform("mozile.xml.load","Failed to load document.");}}catch(e){mozile.debug.inform("mozile.xml.load","Error loading document: "+mozile.dumpError(e));}}
catch(e){mozile.debug.inform("mozile.xml.load","No XML loading technique avaliable in this browser.");}}
return null;}
mozile.xml.parse=function(string){if(window.ActiveXObject){var xmlDoc=new ActiveXObject("Microsoft.XMLDOM");xmlDoc.async="false";xmlDoc.loadXML(string);return xmlDoc;}
...
var nodes=new Array();if(window.XPathEvaluator){var XPE=new XPathEvaluator;var NSE=function(prefix){var namespaceURI=mozile.dom.lookupNamespaceURI(root,prefix);if(namespaceURI)return namespaceURI;else if(mozile.xml.ns[prefix])return mozile.xml.ns[prefix];return mozile.defaultNS;}
try{var results=XPE.evaluate(expression,root,NSE,0,null);if(results){var result=results.iterateNext();while(result){nodes.push(result);result=results.iterateNext();}}}catch(e){alert(doc.documentElement.getAttribute("xmlns")+"\n"+e);}}
else if(root.selectNodes){var results=root.selectNodes(expression);var result=results.nextNode();while(result){nodes.push(result);result=results.nextNode();}}
return nodes;}
mozile.dom.NodeFilter={FILTER_ACCEPT:1,FILTER_REJECT:2,FILTER_SKIP:3,SHOW_ALL:-1,SHOW_ELEMENT:1,SHOW_ATTRIBUTE:2,SHOW_TEXT:4,SHOW_CDATA_SECTION:8,SHOW_ENTITY_REFERENCE:16,SHOW_ENTITY:32,SHOW_PROCESSING_INSTRUCTIONS:64,SHOW_COMMENT:128,SHOW_DOCUMENT:256,SHOW_DOCUMENT_TYPE:512,SHOW_DOCUMENT_FRAGMENT:1024,SHOW_NOTATION:2048}
...
mozile.dom.InternetExplorerSelection.prototype.store=mozile.dom.Selection.prototype.store;mozile.dom.InternetExplorerSelection.prototype.restore=mozile.dom.Selection.prototype.restore;mozile.dom.Selection.restoreSelection=function(r){var range=new mozile.Range();try{range.setStart(r.startContainer,r.startOffset);range.setEnd(r.endContainer,r.endOffset);}catch(e){}
...
mozile.rng.Schema.prototype.load=function(filepath,forceLoad){if(!forceLoad&&this._documents[filepath])return this._documents[filepath];var xmlDoc=mozile.xml.load(filepath);if(xmlDoc)this._documents[filepath]=xmlDoc;return xmlDoc;}
...
mozile.edit=new Object();mozile.edit.prototype=new mozile.Module;mozile.edit.editable=true;mozile.edit.status=false;mozile.edit.NEXT=1;mozile.edit.PREVIOUS=-1;mozile.edit.allCommands=new Object();mozile.edit.keyCodes={8:"Backspace",9:"Tab",12:"Clear",13:"Return",14:"Enter",19:"Pause",27:"Escape",32:"Space",33:"Page-Up",34:"Page-Down",35:"End",36:"Home",37:"Left",38:"Up",39:"Right",40:"Down",45:"Insert",46:"Delete",112:"F1",113:"F2",114:"F3",115:"F4",116:"F5",117:"F6",118:"F7",119:"F8",121:"F9",122:"F10",123:"F11",123:"F12"}
...
mozile.edit.enable=function(){mozile.edit.editable=true;return mozile.edit.editable;}
mozile.edit.disable=function(){mozile.edit.editable=false;return mozile.edit.editable;}
mozile.edit.start=function(){return mozile.edit.setStatus(true);}
mozile.edit.stop=function(){return mozile.edit.setStatus(false);}
mozile.edit.setMark=function(element,key,value){if(!element||element.nodeType==undefined)return null;if(element.nodeType!=mozile.dom.ELEMENT_NODE)return null;if(!key||typeof(key)!="string")return null;try{if(element.mozile==undefined||typeof(element.mozile)!="object")
element.mozile=new Object();element.mozile[key]=value;return value;}catch(e){return null;}}
...
return define;var name=element.getAttribute("name");if(!name)return null;var doc=element.ownerDocument.documentElement;define=null;var child;for(var i=0;i<doc.childNodes.length;i++){child=doc.childNodes[i];if(mozile.dom.getNamespaceURI(child)==mozile.xml.ns.mes&&mozile.dom.getLocalName(child)=="define"&&child.getAttribute("name")==name){define=child;break;}}
if(define){mozile.edit.setMark(element,"define",define);return define;}
else return null;}
mozile.edit.generateCommands=function(schema){var elements=schema.getNodes("element");for(var i=0;i<elements.length;i++){elements[i].addCommand(mozile.edit.navigateLeftRight);if(elements[i].mayContain("text")){elements[i].addCommand(mozile.edit.insertText);elements[i].addCommand(mozile.edit.removeText);}
if(mozile.edit.remove)elements[i].addCommand(mozile.edit.remove);mozile.edit.parseMES(elements[i],elements[i]._element);}}
...
else command=new mozile.edit.Command(name);var child=node.firstChild;while(child){if(child.nodeType==mozile.dom.ELEMENT_NODE){switch(mozile.dom.getLocalName(child)){case"element":var element=mozile.dom.getFirstChildElement(child);if(element)command.element=element;break;case"script":command.script=child;break;}}
child=child.nextSibling;}}
else if(mozile.dom.getLocalName(node)=="group"){command=new mozile.edit.CommandGroup(name);}
else return null;command.node=node;var properties=["priority","label","image","tooltip","accel","makesChanges","watchesChanges","element","text","remove","nested","direction","target","collapse","copyAttributes","styleName","styleValue"];for(var i=0;i<properties.length;i++){var property=properties[i];if(node.getAttribute(property)){var value=node.getAttribute(property);if(value.toLowerCase()=="true")value=true;else if(value.toLowerCase()=="false")value=false;command[property]=value;}}
...
mozile.edit.addCommand=function(command){if(!command)return command;if(!this._commands)this._commands=new Array();if(!this._priority)this._priority=new Array();this._commands.push(command);this._priority.push(command);this._priority.sort(mozile.edit.compareCommands);return command;}
mozile.edit.compareCommands=function(command1,command2){if(command1.priority==undefined||Number(command1.priority)==NaN)
...
mozile.edit.handleEvent=function(event){if(!this._priority)return null;var state;for(var i=0;i<this._priority.length;i++){state=this._priority[i].trigger(event);if(state)return state;}
return null;}
mozile.edit.addDefaultCommand=function(command){if(!mozile.edit._defaultCommands)mozile.edit._defaultCommands=new Array();mozile.edit._defaultCommands.push(command)
return command;}
mozile.edit.handleDefault=function(event){if(!mozile.edit._defaultCommands)return null;var state;for(var i=0;i<mozile.edit._defaultCommands.length;i++){state=mozile.edit._defaultCommands[i].trigger(event);if(state)return state;}
return null;}
...
mozile.rng.Element.prototype.addCommand=mozile.edit.addCommand;mozile.rng.Element.prototype.handleEvent=mozile.edit.handleEvent;}
...
return null;}
mozile.edit.Command.prototype.request=function(state,fresh,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10){var test=this.test(null,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10);if(!test)return null;var newState=this.prepare(null,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10);if(!newState)return null;newState=this.execute(newState,fresh);if(!newState.executed)return null;if(!state.actions)state.actions=new Array();state.actions.push(newState);return newState;}
...
mozile.edit.CommandGroup=function(name){this.name=name;this.group=true;this.makesChanges="none";this.watchesChanges="none";this._commands=new Array();mozile.edit.allCommands[this.name]=this;}
...
mozile.edit.CommandGroup.prototype.addCommand=mozile.edit.addCommand;mozile.edit.CommandGroup.prototype.trigger=mozile.edit.handleEvent;mozile.edit._undoStack=new Array();mozile.edit._undoIndex=-1;mozile.edit.currentState=null;mozile.edit.dumpUndoStack=function(){var entries=new Array("Undo Stack [ "+mozile.edit._undoIndex+" / "+mozile.edit._undoStack.length+" ]");for(var i=0;i<mozile.edit._undoStack.length;i++){var picked="  ";if(i==mozile.edit._undoIndex)picked="> "
...
mozile.edit.save=new mozile.edit.Command("Save");mozile.edit.save.accel="Command-S";mozile.edit.save.image="silk/page_save";mozile.edit.save.makesChanges="none";mozile.edit.save.watchesChanges="state";mozile.edit.addCommand(mozile.edit.save);mozile.edit.save.isAvailable=function(event){if(!mozile.save)return false;if(mozile.save.isSaved())return false;else return true;}
mozile.edit.save.execute=function(state,fresh){mozile.save.save();state.reversible=false;state.executed=true;return state;}
mozile.edit.source=new mozile.edit.Command("Source");mozile.edit.source.image="silk/html";mozile.edit.source.makesChanges="none";mozile.edit.source.watchesChanges="none";mozile.edit.addCommand(mozile.edit.source);mozile.edit.source.execute=function(state,fresh){if(mozile.save&&mozile.gui){var content=mozile.save.getContent(document);content=mozile.save.cleanMarkup(content);mozile.gui.display("<h3>Page Source</h3>\n<pre>"+content+"</pre>");}
state.reversible=false;state.executed=true;return state;}
mozile.edit.debug=new mozile.edit.Command("Debug");mozile.edit.debug.accel="Command-D";mozile.edit.debug.image="silk/bug";mozile.edit.debug.makesChanges="none";mozile.edit.debug.watchesChanges="none";mozile.edit.addCommand(mozile.edit.debug);mozile.edit.debug.execute=function(state,fresh){mozile.debug.show();state.reversible=false;state.executed=true;return state;}
mozile.edit.undo=new mozile.edit.Command("Undo");mozile.edit.undo.accel="Command-Z";mozile.edit.undo.image="silk/arrow_undo";mozile.edit.undo.makesChanges="node";mozile.edit.undo.watchesChanges="state";mozile.edit.addCommand(mozile.edit.undo);mozile.edit.undo.test=function(event){if(mozile.edit._undoIndex<0)return false;if(event){return mozile.edit.checkAccelerator(event,this.accel);}
return true;}
...
mozile.edit.redo=new mozile.edit.Command("Redo");mozile.edit.redo.accel="Command-Shift-Z";mozile.edit.redo.image="silk/arrow_redo";mozile.edit.redo.makesChanges="node";mozile.edit.redo.watchesChanges="state";mozile.edit.addCommand(mozile.edit.redo);mozile.edit.redo.test=function(event){if(mozile.edit._undoIndex+1>=mozile.edit._undoStack.length)return false;if(event){return mozile.edit.checkAccelerator(event,this.accel);}
return true;}
...
mozile.edit.copy=new mozile.edit.Command("Copy");mozile.edit.copy.accel="Command-C";mozile.edit.copy.image="silk/page_copy";mozile.edit.addCommand(mozile.edit.copy);mozile.edit.copy.prepare=function(event){var state=new mozile.edit.State(this,false);state.reversible=false;state.cancel=false;return state;}
...
mozile.edit.cut=new mozile.edit.Command("Cut");mozile.edit.cut.accel="Command-X";mozile.edit.cut.image="silk/cut";mozile.edit.addCommand(mozile.edit.cut);mozile.edit.cut.test=function(event){if(event){if(!event.editable)return false;if(!mozile.edit.checkAccelerator(event,this.accel))return false;if(!mozile.edit.rich){if(event.node)return false;if(event.node.nodeType!=mozile.dom.TEXT_NODE)return false;}}
return true;}
...
mozile.edit.cut.unexecute=function(state,fresh){var selection=mozile.dom.selection.get();for(var i=state.actions.length-1;i>=0;i--){state.actions[i]=state.actions[i].command.unexecute(state.actions[i],fresh);if(state.actions[i].executed)throw("Error: mozile.edit.paste.unexecute Child command unexecute failed at action "+i+".");}
selection.restore(state.selection.before);selection.collapseToEnd();state.executed=false;return state;}
mozile.edit.paste=new mozile.edit.Command("Paste");mozile.edit.paste.accel="Command-V";mozile.edit.paste.image="silk/page_paste";mozile.edit.addCommand(mozile.edit.paste);mozile.edit.paste.test=function(event){if(!mozile.edit.clipboard)return false;if(event){if(!event.editable)return false;if(!mozile.edit.checkAccelerator(event,this.accel))return false;if(!mozile.edit.rich){if(typeof(mozile.edit.clipboard)!="string")return false;if(event.node)return false;if(event.node.nodeType!=mozile.dom.TEXT_NODE)return false;}}
return true;}
...
else{var previousSibling=null;var nextSibling=null;if(selection.focusNode.nodeType==mozile.dom.TEXT_NODE){mozile.edit.splitNode.request(state,fresh);previousSibling=selection.focusNode.previousSibling;nextSibling=selection.focusNode;}
else{previousSibling=selection.focusNode.childNodes[selection.focusOffset-1];nextSibling=selection.focusNode.childNodes[selection.focusOffset];}
var parentNode=null;if(!previousSibling)parentNode=selection.focusNode.parentNode;var clone;if(state.content.childNodes.length>1){clone=state.content.firstChild.cloneNode(true);if(previousSibling&&previousSibling.nodeType==mozile.dom.TEXT_NODE&&clone.nodeType==mozile.dom.TEXT_NODE){selection.collapse(previousSibling,previousSibling.data.length);mozile.edit.insertText.request(state,fresh,null,clone.data);}
else{mozile.edit.insertNode.request(state,fresh,parentNode,previousSibling,clone);previousSibling=clone;parentNode=null;}}
for(var i=1;i<state.content.childNodes.length-1;i++){clone=state.content.childNodes[i].cloneNode(true);mozile.edit.insertNode.request(state,fresh,parentNode,previousSibling,clone);previousSibling=clone;parentNode=null;}
clone=state.content.lastChild.cloneNode(true);if(nextSibling&&nextSibling.nodeType==mozile.dom.TEXT_NODE&&clone.nodeType==mozile.dom.TEXT_NODE){selection.collapse(nextSibling,0);mozile.edit.insertText.request(state,fresh,null,clone.data);}
else{mozile.edit.insertNode.request(state,fresh,parentNode,previousSibling,clone);var IP=mozile.edit.getInsertionPoint(clone,mozile.edit.PREVIOUS);if(IP)IP.select();else{IP=mozile.edit.getInsertionPoint(nextSibling,mozile.edit.NEXT);if(IP)IP.select();else mozile.debug.debug("mozile.edit.paste.execute","Nowhere to collapse to.");}}}
state.selection.after=selection.store();state.executed=true;return state;}
mozile.edit.paste.unexecute=function(state,fresh){var selection=mozile.dom.selection.get();for(var i=state.actions.length-1;i>=0;i--){state.actions[i]=state.actions[i].command.unexecute(state.actions[i],fresh);if(state.actions[i].executed)throw("Error: mozile.edit.paste.unexecute Child command unexecute failed at action "+i+".");}
selection.restore(state.selection.before);selection.collapseToEnd();state.executed=false;return state;}
mozile.edit.test=new mozile.edit.Command("Test");mozile.edit.test.accel="Escape";mozile.edit.addCommand(mozile.edit.test);mozile.edit.test.execute=function(state,fresh){mozile.require("mozile.util");var output=new Array();output.push("Debugging Information:");output.push("Undo: "+mozile.edit._undoIndex+" / "+mozile.edit._undoStack.length);var selection=mozile.dom.selection.get();output.push("Selection:\n"+mozile.util.dumpValues(selection.store()));var element=selection.focusNode;if(element.nodeType!=mozile.dom.ELEMENT_NODE)element=element.parentNode;var rng=mozile.edit.lookupRNG(element);if(rng){if(rng.getName())output.push("RNG: "+rng+" "+rng.getName());else output.push("RNG: "+rng);output.push("Text? "+rng.mayContain("text"));}
else output.push("No matching RNG object.");alert(output.join("\n"));state.reversible=false;state.executed=true;return state;}
mozile.edit.tweak=new mozile.edit.Command("Tweak");mozile.edit.tweak.accel="Command-E";mozile.edit.addCommand(mozile.edit.tweak);mozile.edit.tweak.execute=function(state,fresh){if(mozile.browser.isIE){var selection=mozile.dom.selection.get();var range=selection.getRangeAt(0);range._range.move("character",1);selection.removeAllRanges();selection.addRange(range);}
state.reversible=false;state.executed=true;return state;}
...
mozile.edit.navigateLeftRight.execute=function(state,fresh){var selection=mozile.dom.selection.get();if(selection.isCollapsed||state.extend){var IP=selection.getInsertionPoint();IP.seek(state.direction);if(state.extend)IP.extend();else IP.select();}
else{if(state.direction==mozile.edit.NEXT)selection.collapseToEnd();else selection.collapseToStart();}
state.executed=true;return state;}
mozile.edit.Navigate=function(name){this.name=name;this.group=false;this.remove=true;this.makesChanges="none";this.watchesChanges="none";this.direction="next";this.collapse=null;mozile.edit.allCommands[this.name]=this;}
...
mozile.edit.Navigate.prototype.execute=function(state,fresh){var selection=mozile.dom.selection.get();var target=mozile.xpath.getNode(state.target);var direction=mozile.edit.NEXT;if(this.direction=="previous")direction=mozile.edit.PREVIOUS;var IP=mozile.edit.getInsertionPoint(target,direction);if(IP){IP.select();IP=mozile.edit.getInsertionPoint(target,-1*direction);if(IP)IP.extend();if(this.collapse=="start")selection.collapseToStart();else if(this.collapse=="end")selection.collapseToEnd();var x=mozile.dom.getX(target);var y=mozile.dom.getY(target);var pX=window.pageXOffset;var pY=window.pageYOffset;if(x<pX||x>(pX+window.innerWidth)||y<pY||y>(pY+window.innerHeight)){window.scroll(x-50,y-100);}}
state.executed=true;return state;}
...
else if(mozile.edit.isEmptyToken(selection.focusNode)){state.emptyToken=true;selection.focusNode.data=state.content;selection.collapse(selection.focusNode,selection.focusNode.data.length);state.selection.after=selection.store();}
else if(selection.focusNode.nodeType!=mozile.dom.TEXT_NODE){state.newNode=document.createTextNode(state.content);if(selection.focusOffset==0)mozile.dom.prependChild(state.newNode,selection.focusNode);else selection.focusNode.insertBefore(state.newNode,selection.focusNode.childNodes[selection.focusOffset]);selection.collapse(state.newNode,state.newNode.data.length);state.selection.after=selection.store();}
...
text.insertData(offset,state.content);var newOffset=offset+(state.direction*state.content.length);selection.collapse(text,newOffset);if(state.actions.length==0)
...
selection.restore(state.selection.before);state.executed=false;return state;}
mozile.edit.removeText=new mozile.edit.Command("RemoveText");mozile.edit.removeText.priority=10;mozile.edit.removeText.makesChanges="text";mozile.edit.removeText.watchesChanges="none";mozile.edit.removeText.test=function(event,direction,content){var dir;if(event){if(mozile.edit.checkAccelerator(event,"Backspace")){dir=mozile.edit.PREVIOUS;}
if(mozile.edit.checkAccelerator(event,"Delete")){dir=mozile.edit.NEXT;}
if(!dir)return false;}
if(!dir)dir=mozile.edit.PREVIOUS;if(direction)dir=direction;var selection;if(event&&event.selection)selection=event.selection;if(!selection)selection=mozile.dom.selection.get();if(!selection)return false;var node;if(event&&event.node)node=event.node;if(!node){var range;if(event&&event.range)range=event.range;if(!range)range=selection.getRangeAt(0);if(!range)return false;node=range.commonAncestorContainer;if(event){event.selection=selection;event.range=range;event.node=node;}}
if(!node)return false;if(node.nodeType!=mozile.dom.TEXT_NODE)return false;if(event){if(mozile.edit.remove){if(node.data.length<=2)return false;if(!selection.isCollapsed)return false;}
else if(!selection.isCollapsed)return true;if(dir==mozile.edit.PREVIOUS&&selection.focusOffset>0)return true;if(dir==mozile.edit.NEXT&&selection.focusOffset<node.data.length)return true;return false;}
else return true;}
...
selection.focusNode.deleteData(firstOffset,state.content.length);selection.collapse(selection.focusNode,firstOffset);state.selection.after=selection.store(state.selection.before,firstOffset);}
...
return null;}
mozile.edit.isNodeEditable=function(node){if(!node)return false;if(node.nodeType==mozile.dom.TEXT_NODE){var rng=mozile.edit.lookupRNG(node);if(rng)return rng.mayContain("text");else return true;}
else if(node.nodeType==mozile.dom.ELEMENT_NODE){var rng=mozile.edit.lookupRNG(node);if(rng)return rng.mayContain("text");else{mozile.debug.debug("mozile.edit.isNodeEditable","No RNG Element for element named '"+node.nodeName+"'.");return true;}}
return false;}
...
mozile.edit._getTarget=function(event,target,direction){var node=mozile.edit._getNode(event);if(!node)return null;var test,result;if(!direction)direction="ancestor";if(direction!="ancestor"&&direction!="descendant"&&direction!="next"&&direction!="previous"){mozile.debug.debug("mozile.edit._getTarget","Invalid direction '"+direction+"'.");return null;}
if(typeof(target)=="function"){result=target(event,null);}
...
else if(target.toLowerCase().indexOf("localname")==0){var name=target.substring(9);var match=name.match(/\W*(\w+)\W*/);if(match&&match[1]){name=match[1].toLowerCase();test=function(node){var localName=mozile.dom.getLocalName(node);if(localName&&localName.toLowerCase()==name)return true;else return false;}}
else return null;}
else return null;}
...
if(node&&test(node)&&mozile.edit.isEditable(node)&&mozile.edit.getInsertionPoint(node,mozile.edit.NEXT)){result=node;break;}
if(direction=="ancestor")node=node.parentNode;}}
if(result)return result;else return null;}
mozile.edit.InsertionPoint=function(node,offset){this._node=node;this._offset=offset;}
...
mozile.edit.InsertionPoint.prototype.seek=function(direction){var node=this.getNode();var offset=this.getOffset();if(!node||typeof(offset)=="undefined")return false;if(node.nodeType!=mozile.dom.TEXT_NODE||(direction==mozile.edit.PREVIOUS&&offset==0)||(direction==mozile.edit.NEXT&&offset==node.data.length)||(direction==mozile.edit.NEXT&&mozile.edit.isEmptyToken(node))){return this.seekNode(direction);}
...
var moveBy=0;if(wsLength<2)moveBy=direction;else if(mozile.dom.getStyle(node.parentNode,"white-space").toLowerCase()=="pre")moveBy=direction;else if(wsLength<substring.length)moveBy=wsLength*direction;else if(wsLength==substring.length){return this.seekNode(direction);}
else throw Error("Unhandled case in InsertionPoint.seek()");this._node=node;this._offset=this.getOffset()+moveBy;return true;}
mozile.edit.InsertionPoint.prototype.seekNode=function(direction,extraStep){if(extraStep!==false)extraStep=true;var node=this.getNode();if(!node)return false;var startNode=node;var container=mozile.edit.getContainer(node);if(!container)container=document.documentElement;var treeWalker=document.createTreeWalker(container,mozile.dom.NodeFilter.SHOW_ALL,null,false);if(node.nodeType==mozile.dom.TEXT_NODE){treeWalker.currentNode=node;if(direction==mozile.edit.NEXT)node=treeWalker.nextNode();else{var tempNode=node;node=treeWalker.previousNode();while(node&&node.firstChild==tempNode){tempNode=node;node=treeWalker.previousNode();}}}
else{node=node.childNodes[this.getOffset()];if(node){startNode=node;treeWalker.currentNode=node;}
else if(this.getOffset()>0&&this.getOffset()==this.getNode().childNodes.length&&this.getNode().childNodes[this.getOffset()-1]){node=this.getNode().childNodes[this.getOffset()-1];startNode=node;treeWalker.currentNode=node;}
else{node=this.getNode();startNode=node;treeWalker.currentNode=node;if(direction==mozile.edit.NEXT)node=treeWalker.nextNode();else{var tempNode=node;node=treeWalker.previousNode();while(node&&node.firstChild==tempNode){tempNode=node;node=treeWalker.previousNode();}}}}
if(!node){mozile.debug.debug("mozile.edit.InsertionPoint.prototype.seekNode","Lost node.");return false;}
var IP;var offset=null;var lastNode=node;var skippedOne=false;while(node){IP=mozile.edit.getInsertionPoint(node,direction);if(IP){this._node=IP.getNode();this._offset=IP.getOffset();if(extraStep&&(mozile.edit.getParentBlock(node)==mozile.edit.getParentBlock(startNode)&&mozile.edit.isNodeEditable(lastNode))||(lastNode.nodeType==mozile.dom.COMMENT_NODE&&node==IP.getNode()))
this._offset=this._offset+direction;if(mozile.edit.isEmptyToken(IP.getNode()))this._offset=0;return true;}
else if(node.nodeType==mozile.dom.ELEMENT_NODE&&mozile.edit.isNodeEditable(node.parentNode)){var nextNode=node;while(nextNode){if(direction==mozile.edit.NEXT)nextNode=nextNode.nextSibling;else nextNode=nextNode.previousSibling;if(nextNode&&nextNode.nodeType==mozile.dom.COMMENT_NODE)continue;else break;}
IP=mozile.edit.getInsertionPoint(nextNode,direction);if(!IP){var newNode=node;if(direction==mozile.edit.PREVIOUS){if(nextNode.parentNode==this._node&&!skippedOne){skippedOne=true;lastNode=node;node=nextNode;treeWalker.currentNode=node;continue;}
newNode=nextNode;}
this._node=newNode.parentNode;this._offset=mozile.dom.getIndex(newNode)+1;return true;}}
lastNode=node;if(direction==mozile.edit.NEXT)node=treeWalker.nextNode();else node=treeWalker.previousNode();}
return false;}
mozile.dom.Selection.prototype.getInsertionPoint=function(){if(!this.focusNode||this.focusOffset==null)return null;else return new mozile.edit.InsertionPoint(this.focusNode,this.focusOffset);}
...
mozile.edit.getInsertionPoint=function(node,direction,force){if(!node)return false;var offset,IP;if(mozile.edit.isNodeEditable(node)||force){if(node.nodeType==mozile.dom.TEXT_NODE){if(direction==mozile.edit.NEXT)offset=0;else offset=node.data.length;return new mozile.edit.InsertionPoint(node,offset);}
...
mozile.edit.remove=new mozile.edit.Command("Remove");mozile.edit.remove.test=function(event,direction,content,preserve){if(event){if(mozile.edit.checkAccelerators(event,["Backspace","Delete"]))return true;return false;}
return true;}
...
mozile.edit.remove.execute=function(state,fresh){var selection=mozile.dom.selection.get();if(!fresh)selection.restore(state.selection.before);state.actions=new Array();var newState,IP;var collapsed=selection.isCollapsed;if(selection.isCollapsed){var node=selection.focusNode;if(node&&node.nodeType==mozile.dom.TEXT_NODE&&!mozile.edit.isEmptyToken(node)){if((state.direction==mozile.edit.PREVIOUS&&selection.focusOffset>0)||(state.direction==mozile.edit.NEXT&&selection.focusOffset<node.data.length)){mozile.edit.removeText.request(state,fresh,state.direction);}
else{IP=selection.getInsertionPoint();IP.seek(state.direction);IP.extend();}}
else{IP=selection.getInsertionPoint();IP.seek(state.direction);IP.extend();}}
if(!selection.isCollapsed){if(selection.getRangeAt(0).commonAncestorContainer.nodeType==mozile.dom.TEXT_NODE){mozile.edit.removeText.request(state,fresh,state.direction);}
...
mozile.edit.Insert.prototype=new mozile.edit.Command;mozile.edit.Insert.prototype.constructor=mozile.edit.Insert;mozile.edit.Insert.prototype.prepare=function(event){var state=new mozile.edit.State(this);state.element=null;if(typeof(this.element)=="string"){state.element=mozile.dom.createElement(this.element);}
...
return state;}
mozile.edit.Insert.prototype.execute=function(state,fresh){var selection=mozile.dom.selection.get();if(!fresh)selection.restore(state.selection.before);state.actions=new Array();var newState;if(this.remove&&!selection.isCollapsed){mozile.edit.remove.request(state,fresh,mozile.edit.NEXT);selection=mozile.dom.selection.get();}
...
if(selection.isCollapsed){var previousNode;if(selection.focusNode.nodeType==mozile.dom.TEXT_NODE){newState=mozile.edit.splitNode.request(state,fresh,selection.focusNode,selection.focusOffset);previousNode=newState.oldContainer;}
else previousNode=selection.focusNode[selection.focusOffset];mozile.edit.insertNode.request(state,fresh,null,previousNode,state.element);if(!this.remove){var text;if(mozile.edit.isBlock(state.element))
...
var IP=mozile.edit.getInsertionPoint(state.element,mozile.edit.NEXT);if(IP){if(this.remove){IP.seekNode(mozile.edit.NEXT,false);if(IP)IP.select();}
...
return wrapper;}
mozile.edit.Wrap.prototype.isActive=function(event){if(this.prompt)return false;if(this._getWrapper(event.node))return true;else return false;}
mozile.edit.Wrap.prototype.prepare=function(event){var state=new mozile.edit.State(this);state.wrapper=null;if(typeof(this.element)=="string"){state.wrapper=mozile.dom.createElement(this.element);if(this.styleName){mozile.dom.setStyle(state.wrapper,this.styleName,this.styleValue);}}
else if(this.element&&this.element.cloneNode){state.wrapper=this.element.cloneNode(true);}
if(this.prompt){if(!this.prompt(event,state))return null;}
return state;}
mozile.edit.Wrap.prototype.execute=function(state,fresh){var selection=mozile.dom.selection.get();if(!fresh)selection.restore(state.selection.before);var range=selection.getRangeAt(0);state.actions=new Array();var wrapper=state.wrapper;state.wrappers=new Array();var nextNode,previousNode,textNode,newState;if(range.collapsed){var outerWrapper=this._getWrapper(range.commonAncestorContainer,true);if(outerWrapper&&!this.nested){if(selection.focusNode==outerWrapper.lastChild&&selection.focusOffset==selection.focusNode.data.length){var IP=mozile.edit.getInsertionPoint(outerWrapper,mozile.edit.PREVIOUS);IP.seekNode(mozile.edit.NEXT,false);IP.select();mozile.edit._removeEmpty(state,fresh,outerWrapper.lastChild,outerWrapper.parentNode);}
else{newState=mozile.edit.splitNodes.request(state,fresh,range.startContainer,range.startOffset,outerWrapper.parentNode);nextNode=newState.newContainer;textNode=document.createTextNode("");mozile.edit.insertNode.request(state,fresh,nextNode.parentNode,nextNode.previousSibling,textNode);selection.collapse(textNode,0);}}
else{if(range.startContainer.nodeType==mozile.dom.TEXT_NODE){newState=mozile.edit.splitNodes.request(state,fresh,range.startContainer,range.startOffset,range.startContainer.parentNode);nextNode=newState.newContainer;}
else nextNode=range.startContainer.childNodes[range.startOffset];mozile.edit.insertNode.request(state,fresh,nextNode.parentNode,nextNode.previousSibling,wrapper);wrapper.appendChild(document.createTextNode(""));selection.collapse(wrapper.firstChild,0);}}
...
selection=mozile.dom.selection.get();range=selection.getRangeAt(0);container=range.commonAncestorContainer;if(container.nodeType!=mozile.dom.TEXT_NODE){var IP=mozile.edit.getInsertionPoint(previousNode,mozile.edit.PREVIOUS);if(IP){IP.seekNode(mozile.edit.NEXT,false);IP.select();IP=mozile.edit.getInsertionPoint(nextNode,mozile.edit.NEXT);if(IP){IP.seekNode(mozile.edit.PREVIOUS,false);IP.extend();}}}
...
mozile.edit.Replace.prototype.prepare=function(event){var state=new mozile.edit.State(this);state.element=null;if(typeof(this.element)=="string"){state.element=mozile.dom.createElement(this.element);}
else if(this.element&&this.element.cloneNode){state.element=this.element.cloneNode(true);}
...
if(target.className)state.element.className=target.className;if(target.mozile){state.element.mozile={};for(var key in start.target.mozile){state.element.mozile[key]=target.mozile[key];}}}
if(this.prompt){if(!this.prompt(event,state))return null;}
return state;}
...
return state;}
mozile.edit.Style.prototype.execute=function(state,fresh){var target=mozile.xpath.getNode(state.target);state.targetNode=target;state.oldValue=target.style[state.styleName];target.style[state.styleName]=state.styleValue;state.executed=true;return state;}
mozile.edit.Style.prototype.unexecute=function(state,fresh){state.targetNode.style[state.styleName]=state.oldValue;state.executed=false;return state;}
...
mozile.edit._normalize=function(state,fresh,firstNode,secondNode){if(!firstNode||!firstNode.parentNode||firstNode.nodeType!=mozile.dom.TEXT_NODE){return false;}
if(!secondNode||!secondNode.parentNode||secondNode.nodeType!=mozile.dom.TEXT_NODE){return false;}
if(firstNode.nextSibling!=secondNode)return false;return mozile.edit.mergeNodes.request(state,fresh,firstNode,secondNode);}
...
state.direction=mozile.edit.PREVIOUS;var IP=mozile.edit.getInsertionPoint(target,-1*state.direction);if(!IP)return null;var result=IP.seekNode(-1*state.direction,false);if(!result)IP.seekNode(state.direction,false);if((target.nodeType==mozile.dom.TEXT_NODE&&target.data.length==0)||(target.nodeType==mozile.dom.ELEMENT_NODE&&target!=mozile.edit.getContainer(target)&&target.childNodes.length==0)){if(target==limitNode){var content=mozile.edit.createEmptyToken();mozile.edit.insertNode.request(state,fresh,target,null,content);selection.collapse(content,0);return null;}
...
else IP.select();var firstNode,secondNode;if(state.direction==mozile.edit.PREVIOUS){secondNode=IP.getNode();IP.seekNode(state.direction,false);firstNode=IP.getNode();}
...
mozile.event=new Object();mozile.event.prototype=new mozile.Module;mozile.event.mousedown=false;mozile.event.normalize=function(event){if(!event)return event;if(typeof(event.target)=="undefined")event.target=event.srcElement;if(typeof(event.charCode)=="undefined")event.charCode=event.keyCode;return event;}
...
event=mozile.event.normalize(event);mozile.event.findTarget(event);if(!event.node)return true;event.container=mozile.edit.getContainer(event.node);event.editable=Boolean(event.container);var state=mozile.edit.handleEvent(event);if(state)return mozile.event.handled(event,state);if(mozile.gui)mozile.gui.update(event);if(!mozile.edit.editable)return true;var priorStatus=mozile.edit.status;mozile.edit.setStatus(event.editable);if(!event.editable)return true;mozile.event.storeSelection(event);if(!priorStatus)mozile.event.fixFocus(event);var node=event.node;while(event.node){if(!event.rng)event.rng=mozile.edit.lookupRNG(event.node);if(event.rng){state=event.rng.handleEvent(event);if(state)return mozile.event.handled(event,state);}
if(event.node==event.container)break;else event.node=event.node.parentNode;}
event.node=node;state=mozile.edit.handleDefault(event);if(state)return mozile.event.handled(event,state);if(!mozile.event.cancelKeyEvent(event))return false;if(!mozile.event.cancelHyperlink(event))return false;}catch(e){mozile.debug.inform("mozile.event.handle",mozile.dumpError(e));}
return true;}
mozile.event.handled=function(event,state){mozile.edit.done(state);if(state.changesMade){if(mozile.gui)mozile.gui.update(event,state.changesMade);}
...
mozile.event.cancelKeyEvent=function(event){if(!event||!event.keyCode)return true;switch(event.keyCode){case 8:case 46:mozile.event.cancel(event);return false;}
return true;}
...
mozile.gui.htmlToolbar=new mozile.gui.Factory("HTMLToolbarFactory");mozile.gui.factory=mozile.gui.htmlToolbar;mozile.gui.htmlToolbar.toolbar=null;mozile.gui.htmlToolbar.checkmark="\u2713";mozile.gui.htmlToolbar.ieColor="#C2C2C2";mozile.gui.htmlToolbar.ieActiveColor="#C1D2EE";mozile.gui.htmlToolbar.ieMenuWidth="170px";mozile.gui.htmlToolbar.ieBorder="";mozile.gui.htmlToolbar.ieActiveBorder="1px solid #316AC5";mozile.gui.htmlToolbar.iePadding="1px";mozile.gui.htmlToolbar.ieActivePadding="0px";mozile.gui.htmlToolbar.about=new mozile.edit.Command("About");mozile.gui.htmlToolbar.about.label="About Mozile";mozile.gui.htmlToolbar.about.image="silk/information";mozile.gui.htmlToolbar.about.execute=function(state,fresh){alert(["About Mozile "+mozile.version,mozile.homepage,mozile.about,mozile.copyright+" "+mozile.license,"Contributors: "+mozile.credits,"Acknowledgements: "+mozile.acknowledgements].join("\n"));state.reversible=false;state.executed=true;return state;}
mozile.gui.htmlToolbar.help=new mozile.edit.Command("Help");mozile.gui.htmlToolbar.help.image="silk/help";mozile.gui.htmlToolbar.help.execute=function(state,fresh){window.open(mozile.help,"MozileHelp","");state.reversible=false;state.executed=true;return state;}
...
mozile.gui.htmlToolbar.createImage=function(name){if(!name||typeof(name)!="string")return null;var filetype=".png";if(name.indexOf(".")>-1)filetype="";var img;var src=[mozile.root,"images",name+filetype].join(mozile.filesep);if(false&&mozile.browser.isIE&&(filetype==".png"||name.toLowerCase().substring(name.length-3,name.length)=="png")){img=mozile.gui.htmlToolbar.createElement("span");img.style.cssText="width: 16px; height: 16px; display:inline-block; "
+"filter:progid:DXImageTransform.Microsoft.AlphaImageLoader"
+"(src=\'"+src+"\', sizingMethod='image');";}
else{img=mozile.gui.htmlToolbar.createElement("img");img.setAttribute("src",src);}
return img;}
mozile.gui.htmlToolbar.create=function(){if(this.toolbar)return;var href=[mozile.root,"src","gui","htmlToolbar.css"].join(mozile.filesep);mozile.dom.addStyleSheet(href,"text/css");this.toolbar=mozile.gui.htmlToolbar.createElement("div");this.toolbar.setAttribute("id","mozileToolbar");if(mozile.browser.isIE)this.toolbar.className="mozileGUI";else this.toolbar.setAttribute("class","mozileGUI");var body=mozile.dom.getBody();body.appendChild(this.toolbar);if(mozile.browser.isIE){mozile.dom.setStyle(this.toolbar,"background-color",this.ieColor);}
this.updateButtons();this.reposition();if(!mozile.browser.isMozilla||!mozile.useDesignMode){mozile.gui.htmlToolbar.interval=window.setInterval("mozile.gui.htmlToolbar.reposition()",mozile.updateInterval);}}
mozile.gui.htmlToolbar.reposition=function(){if(!mozile.gui.htmlToolbar.toolbar)mozile.gui.htmlToolbar.create();if(mozile.browser.isMozilla){mozile.dom.setStyle(mozile.gui.htmlToolbar.toolbar,"position","fixed");mozile.dom.setStyle(mozile.gui.htmlToolbar.toolbar,"top","-1px");}
...
mozile.gui.htmlToolbar.update=function(event,change){if(!event)return false;if(!change){if(event.type.indexOf("key")>-1&&event.type!="keyup")return false;}
this.reposition();if(!event||!event.node){mozile.gui.htmlToolbar.closeMenus();return false;}
if(!change)change="none";if(change=="node"||event.node!=this.lastNode){change="node";if(mozile.dom.isAncestorOf(this.toolbar,event.node))return false;else mozile.gui.htmlToolbar.closeMenus();this.lastNode=event.node;if(!event.rng)event.rng=mozile.edit.lookupRNG(event.node);if(!event.rng)return false;this.updateButtons(event.rng,event);}
this.updateCommands(event,change);this.reposition();return true;}
mozile.gui.htmlToolbar.updateButtons=function(rng,event){mozile.dom.removeChildNodes(this.toolbar);this.toolbar.commands=new Array();for(var i=0;i<mozile.gui.htmlToolbar._commands.length;i++){this.updateButton(mozile.gui.htmlToolbar._commands[i]);}
this.toolbar.appendChild(document.createTextNode("|"));for(var i=0;i<mozile.edit._commands.length;i++){this.updateButton(mozile.edit._commands[i]);}
if(rng&&rng._commands&&rng._commands.length){this.toolbar.appendChild(document.createTextNode("|"));for(var i=0;i<rng._commands.length;i++){this.updateButton(rng._commands[i],event);}}}
...
mozile.gui.htmlToolbar.Button=function(command){if(!command)return;this.command=command;this.type="Button";this.parent=null;this.element=mozile.gui.htmlToolbar.createElement("span");this.element.setAttribute("class","mozileButton");this.element.onclick=function(event){var c=mozile.edit.getCommand(command.name);if(c)c.button.press(event);}
this.image=mozile.gui.htmlToolbar.createImage(command.image);this.element.appendChild(this.image);if(command.group){this.menu=new mozile.gui.htmlToolbar.Menu(command);this.menu.parent=this;this.element.appendChild(this.menu.element);var img=mozile.gui.htmlToolbar.createImage("arrow-down.gif");this.element.appendChild(img);}
command.button=this;}
mozile.gui.htmlToolbar.Button.prototype.press=function(event){mozile.event.normalize(event);var selection=mozile.dom.selection.get();selection.restore();var opened=false;if(this.menu&&this.menu.opened)opened=true;if(!this.parent)mozile.gui.htmlToolbar.closeMenus();if(this.menu){if(opened)this.menu.close();else this.menu.open(event);}
else{mozile.gui.htmlToolbar.closeMenus();var command=mozile.edit.getCommand(this.command.name);if(command){if(!command.test()){mozile.debug.debug("mozile.gui.htmlToolbar.Button.press","Command '"+command.name+"' failed its test and will not be executed.");return;}
var state=command.prepare(event);if(!state)return;state=command.execute(state,false);if(state)mozile.edit.done(state);else alert("The execution of the command '"+command.name+"' did not return a state.");if(mozile.browser.isMozilla){var target=selection.focusNode;var newEvent=document.createEvent("MouseEvents");newEvent.initMouseEvent("click",true,true,window,1,0,0,mozile.dom.getX(target),mozile.dom.getY(target),false,false,false,false,1,null);target.dispatchEvent(newEvent);if(state&&state.selection){if(state.selection.after)selection.restore(state.selection.after);else selection.restore(state.selection.before);}
...
mozile.gui.htmlToolbar.MenuItem=function(command){if(!command)return;this.command=command;this.type="MenuItem";this.parent=null;this.element=mozile.gui.htmlToolbar.createElement("tr");this.element.setAttribute("class","mozileMenuItem");this.element.onclick=function(event){var c=mozile.edit.getCommand(command.name);if(c)c.menuItem.press(event);}
this.cells={};this.cells.active=mozile.gui.htmlToolbar.createElement("td");this.cells.icon=mozile.gui.htmlToolbar.createElement("td");this.cells.label=mozile.gui.htmlToolbar.createElement("td");this.cells.accel=mozile.gui.htmlToolbar.createElement("td");if(mozile.browser.isIE){this.cells.active.className="mozileActive";this.cells.icon.className="mozileIcon";this.cells.label.className="mozileLabel";this.cells.accel.className="mozileAccel";}
